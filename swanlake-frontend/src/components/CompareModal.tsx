...
export default function CompareModal({ isOpen, onClose, currentProduct }: CompareModalProps) {
    ...
    const fetchComparableProducts = async (productType: string) => {
        try {
            ...
            const filteredProducts = products.filter(
                (prod) => prod.productType === productType && prod.reviewID !== currentProduct.reviewID
            );
    ...
    const handleProductSelection = (e: React.ChangeEvent<HTMLSelectElement>) => {
        ...
        const product = comparableProducts.find(prod => prod.reviewID === selectedReviewID);
        if (product) {
                ...
                <div className="relative p-6">
                    ...
                    {loading ? (
                        <div className="text-center py-8">
                            ...
                            <p className="mt-2 text-gray-600 dark:text-gray-300">Loading comparable products...</p>
                        </div>
                    ) : (
                                ...
                                {comparableProducts.length > 0 ? (
                                    <>
                                        ...
                                        {selectedProduct ? (
                                            ...
                                            <div className="text-center py-8">
                                                <p className="dark:text-white">Please select a product to compare</p>
                                            </div>
                                        )}
                                    </>
                                ) : (
                                    <div className="text-center py-8">
                                        <p className="dark:text-white">No comparable products available</p>
                                    </div>
                                )}
                            </div>
                        </div>
